...
- `mail-templates/` — MJML sources for Contact Form 7 message bodies, one folder per form (`register/` has both the staff-notification and auto-reply templates). Each template exists in a `-pt`/`-en` pair (MJML has no built-in i18n) with its `.mjml` source(s) and built `.html`. `_partials/head.mjml` centralizes the shared `mj-body`/`mj-wrapper` defaults, dark-mode/format-detection meta tags, and `mj-class` presets (e.g. `pb-label`, `pb-value`, `pb-row`, `pb-title`) reused across every template — change a style once there instead of per-template. Other partials are language-neutral (`header.mjml`, `spacer.mjml`) or split into `-pt`/`-en` pairs (footer, footer-fibaq, header-fibaq, credits). `legacy/` holds superseded HTML-only templates kept for reference. Run `npm run build` (or `npm run build:<name>-pt`/`-en`) inside `mail-templates/` to rebuild the `.html` files after editing a `.mjml` source — paste the built HTML into CF7's mail body field.
