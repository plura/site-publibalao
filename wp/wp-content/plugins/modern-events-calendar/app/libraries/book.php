...
class MEC_book extends MEC_base
{
    ...
    public function get_price_details($tickets, $event_id, $event_tickets, $variations = array(), $other_dates = array(), $apply_fees = true)
    {
        ...
        for($c = 1; $c <= $dates_count; $c++)
        {
            ...
            foreach($tickets as $ticket_id=>$count)
            {
                ...
                if(isset($this->settings['ticket_variations_status']) and $this->settings['ticket_variations_status'] and is_array($variations) and count($variations))
                {
                    ...
                    foreach($ticket_variations as $key=>$ticket_variation)
                    {
                        ...
                        // Price Details
                        if(!isset($variation_details[$key])) $variation_details[$key] = array('amount'=>$variation_amount, 'description'=>__($variation_title, 'mec'), 'type'=>'variation', 'count' => $variation_count);
                        else
                        {
                            ...
                            $variation_details[$key]['description'] = __($ticket_variation['title'].' ('.$new_count.')', 'mec');
                        }
                    }
                }
            }
            ...
            if($apply_fees and isset($this->settings['taxes_fees_status']) and $this->settings['taxes_fees_status'])
            {
                ...
                foreach($fees as $key=>$fee)
                {
                    ...
                    // Price Details
                    if(!isset($fee_details[$key])) $fee_details[$key] = array('amount'=>$fee_amount, 'description'=>__($fee['title'], 'mec'), 'type'=>'fee', 'fee_type'=>$fee['type'], 'fee_amount'=>$fee['amount']);
        ...
        // Ticket Details
        $details[] = array('amount'=>$total_tickets_amount, 'description'=>sprintf(__('%s Price', 'mec'), $this->main->m('tickets', __('Tickets', 'mec'))), 'type'=>'tickets');
