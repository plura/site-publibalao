...
class MEC_main extends MEC_base
{
    ...
    public function get_sidebar_menu($active_menu = 'settings')
    {
        ...
        $settings = apply_filters('mec-settings-items-settings', array(
            __('General', 'mec') => 'general_option',
            __('Archive Pages', 'mec') => 'archive_options',
            __('Slugs/Permalinks', 'mec') => 'slug_option',
            __('Currency', 'mec') => 'currency_option',
            __('Assets Per Page', 'mec') => 'assets_per_page_option',
            __('Google Recaptcha', 'mec') => 'recaptcha_option',
            ...
            __('Email', 'mec') => 'email_option',
        ), $active_menu);

        $integrations = apply_filters('mec-settings-items-integrations', array(
            __('Mailchimp', 'mec') => 'mailchimp_option',
            __('Campaign Monitor', 'mec') => 'campaign_monitor_option',
            __('MailerLite', 'mec') => 'mailerlite_option',
            __('Constant Contact', 'mec') => 'constantcontact_option',
            __('Active Campaign', 'mec') => 'active_campaign_option',
            __('AWeber', 'mec') => 'aweber_option',
            __('MailPoet', 'mec') => 'mailpoet_option',
            __('Sendfox', 'mec') => 'sendfox_option',
            __('BuddyPress', 'mec') => 'buddy_option',
            __('LearnDash', 'mec') => 'learndash_options',
            __('PaidMembership Pro', 'mec') => 'pmp_options',
        ), $active_menu);
        ...
        $single_event = apply_filters('mec-settings-item-single_event', array(
            ...
            __('Countdown', 'mec') => 'countdown_option',
        ...
        $booking = apply_filters('mec-settings-item-booking', array(
            $this->m('booking', __('Booking', 'mec')) => 'booking_option',
            sprintf(__('%s Shortcode', 'mec'), $this->m('booking', __('Booking', 'mec'))) => 'booking_shortcode',
            __('Coupons', 'mec') => 'coupon_option',
            __('Taxes / Fees', 'mec') => 'taxes_option',
            __('Ticket Variations & Options', 'mec') => 'ticket_variations_option',
            sprintf(__('%s Form', 'mec'), $this->m('booking', __('Booking', 'mec'))) => 'booking_form_option',
        ...
        $modules = apply_filters('mec-settings-item-modules', array(
            ...
            __('Map', 'mec') => 'googlemap_option',
            __('Export', 'mec') => 'export_module_option',
        ...
        if($this->getPro())
        {
            $notifications_items = array_merge(array(
                ...
                __('Event Finished', 'mec') => 'event_finished',
            ), $notifications_items);

            $notifications_items[__('Notifications Per Event', 'mec')] = 'notifications_per_event';
            $notifications_items[__('Notification Template', 'mec')] = 'notification_template';
            $single_event[__('Edit Per Occurrences', 'mec')] = 'per_occurrences';
            $single_event[__('Only For Bookers', 'mec')] = 'shortcode_only_bookers';
            $modules[__('Auto Emails', 'mec')] = 'auto_emails_option';
        }
        ...
        <ul class="wns-be-group-menu">
            ...
            <li class="wns-be-group-menu-li mec-settings-menu <?php echo $active_menu == 'integrations' ? 'active' : ''; ?>">
                <a href="<?php echo $this->add_qs_var('tab', 'MEC-integrations'); ?>" id="" class="wns-be-group-tab-link-a">
                    ...
                    <span class="wns-be-group-menu-title"><?php  _e('Integrations', 'mec'); ?></span>
                </a>
            ...
            <li class="wns-be-group-menu-li mec-settings-menu <?php echo $active_menu == 'booking' ? 'active' : ''; ?>">
                <a href="<?php echo $this->add_qs_var('tab', 'MEC-booking'); ?>" id="" class="wns-be-group-tab-link-a">
                    ...
                    <span class="wns-be-group-menu-title"><?php echo $this->m('booking', __('Booking', 'mec')); ?></span>
                </a>
    ...
    public function get_archive_skins()
    {
        $archive_skins = array(
            ...
            // array('skin'=>'general_calendar', 'name'=>__('General Calendar', 'mec')),
        ...
        if(isset($_GET['method']) and sanitize_text_field($_GET['method']) == 'mec-invoice')
        {
            ...
            if(count($all_dates))
            {
                ...
                foreach($all_dates as $one_date)
                {
                    ...
                    {
                        $pdf->Write(6, sprintf(__('%s to %s', 'mec'), $this->date_i18n($date_format.' '.$time_format, $other_timestamps[0]), $this->date_i18n($date_format.' '.$time_format, $other_timestamps[1])));
                        $pdf->Ln();
                    }
                }
            ...
            if(is_array($bfixed_fields) and count($bfixed_fields) and isset($transaction['fields']) and is_array($transaction['fields']) and count($transaction['fields']))
            {
                ...
                $pdf->Write(20, sprintf(__('%s Fields', 'mec'), $this->m('booking', __('Booking', 'mec'))));
                $pdf->Ln();
        ...
        $field = '<li id="mec_'.$prefix.'_fields_'.$key.'">
            ...
            <p class="mec_'.$prefix.'_field_options">
                <label>
                    ...
                    '.__('Consider the first item as a placeholder', 'mec').'
                </label>
            </p>
            <span onclick="mec_'.$prefix.'_fields_remove('.$key.');" class="mec_'.$prefix.'_field_remove">'.__('Remove', 'mec').'</span>
            <div>
    ...
    */
    ...
    public function get_messages()
    {
        if($this->getPRO())
        {
            $messages = array(
                ...
                'booking'=>array(
                    'category'=>array('name'=>__('Booking', 'mec')),
                    'messages'=>array(
                        'booking'=>array('label'=>__('Booking (Singular)', 'mec'), 'default'=>__('Booking', 'mec')),
                        'bookings'=>array('label'=>__('Bookings (Plural)', 'mec'), 'default'=>__('Bookings', 'mec')),
    ...
    public function get_flags($event, $date = NULL)
    {
        ...
        if(is_array($availability) and count($availability))
        {
            ...
            if(($total_bookings_limit > 0) and ($remained_tickets > 0 and $remained_tickets <= (($percentage * $total_bookings_limit) / 100))) return str_replace('%%title%%', __('Last Few Tickets', 'mec'), $output_tag);
    ...
    public function get_normal_labels($event, $display_label = false)
    {
        ...
        // Expired Event
        elseif($display_label and $this->is_expired($event)) $output .= '<span data-style="Normal" class="mec-label-normal mec-expired-normal-label">' . esc_html__('Expired', 'mec') . '</span>';
    ...
    {
        if($this->is_ongoing($event)) return '<dl><dd><span class="mec-holding-status mec-holding-status-ongoing">'.__('Ongoing...', 'mec').'</span></dd></dl>';
        elseif($this->is_expired($event)) return '<dl><dd><span class="mec-holding-status mec-holding-status-expired">'.__('Expired!', 'mec').'</span></dd></dl>';
