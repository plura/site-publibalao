...
class MEC_skins extends MEC_base
{
    ...
    public function sf_search_form()
    {
        ...
        if(trim($fields) && (in_array('dropdown', $display_form) || in_array('checkboxes', $display_form) || in_array('text_input', $display_form) || in_array('address_input', $display_form) || in_array('minmax', $display_form) || in_array('local-time-picker', $display_form)))
        {
            ...
            // Reset Button
            if($this->sf_reset_button) $form .='<div class="mec-search-reset-button"><button class="button mec-button" id="mec_search_form_'.$this->id.'_reset" type="button">'.esc_html__('Reset', 'mec').'</button></div>';
    ...
    public function sf_search_field($field, $options , $display_label = null)
    {
        ...
        if($field == 'category')
        {
            $label = $this->main->m('taxonomy_category', __('Category', 'mec'));
        ...
        {
            $label = $this->main->m('taxonomy_location', __('Location', 'mec'));
        ...
        {
            $label = $this->main->m('taxonomy_organizer', __('Organizer', 'mec'));
        ...
        {
            $label = $this->main->m('taxonomy_speaker', __('Speaker', 'mec'));
        ...
        elseif($field == 'tag')
        {
            $label = $this->main->m('taxonomy_tag', __('Tag', 'mec'));
        ...
        elseif($field == 'label')
        {
            $label = $this->main->m('taxonomy_label', __('Label', 'mec'));
            ...
            if($type == 'dropdown')
            {
                ...
                $output .= '<i class="mec-sl-calendar"></i>
                    <select id="mec_sf_month_'.$this->id.'">
                        <option value="">'.__('Select Month','mec').'</option>';
    ...
    public function display_detailed_time($event)
    {
        ...
        $output .= '<div class="mec-detailed-time-start">'.sprintf(__('Start from: %s - %s', 'mec'), date_i18n($date_format, strtotime($from)), date_i18n($time_format, strtotime($from.' '.$start_time))).'</div>';
        $output .= '<div class="mec-detailed-time-end">'.sprintf(__('End at: %s - %s', 'mec'), date_i18n($date_format, strtotime($to)), date_i18n($time_format, strtotime($to.' '.$end_time))).'</div>';
        $output .= '</div>';
