	...
	public function output( $event_id = 0, $atts = array() ){
		...
		if ( true === $this->is_editor_mode && ( !isset($settings['social_network_status']) || !$settings['social_network_status'] ) ) {
			...
			$html = '<div class="mec-content-notification"><p>'
				.'<span>'. __('This widget is displayed if social networks is set. In order for the widget in this page to be displayed correctly, please set label for your last event.', 'mec').'</span>'
				. '<a href="https://webnus.net/dox/modern-events-calendar/social-networks/" target="_blank">' . __('How to set social networks', 'mec') . ' </a>'
			.'</p></div>';
		} else {
