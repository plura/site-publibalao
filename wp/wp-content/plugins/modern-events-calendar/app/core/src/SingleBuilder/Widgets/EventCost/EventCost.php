	...
	public function output( $event_id = 0, $atts = array() ){
		...
		if ( true === $this->is_editor_mode && !(isset($events_detail->data->meta['mec_cost']) && $events_detail->data->meta['mec_cost'] != '') ) {
			...
			$html = '<div class="mec-content-notification"><p>'
					.'<span>'. __('This widget is displayed if cost is set. In order for the widget in this page to be displayed correctly, please set cost for your last event.', 'mec').'</span>'
					. '<a href="https://webnus.net/dox/modern-events-calendar/add-event/" target="_blank">' . __('How to set cost', 'mec') . ' </a>'
				.'</p></div>';
		} else {
			...
			if (isset($events_detail->data->meta['mec_cost']) && $events_detail->data->meta['mec_cost'] != '') {
				...
				<div class="mec-event-meta">
					<div class="mec-event-cost">
						...
						<h3 class="mec-cost"><?php echo Base::get_main()->m('cost', __('Cost', 'mec')); ?></h3>
