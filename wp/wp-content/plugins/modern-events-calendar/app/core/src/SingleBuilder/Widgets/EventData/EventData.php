	...
	public function output( $event_id = 0, $atts = array() ){
		...
		if ( true === $this->is_editor_mode && ( empty($data) || ( isset($settings['display_event_fields']) && !$settings['display_event_fields'] ) ) ) {
			...
			$html = '<div class="mec-content-notification"><p>'
					.'<span>'. __('This widget is displayed if event data is set. In order for the widget in this page to be displayed correctly, please set event data for your last event.', 'mec').'</span>'
					. '<a href="https://webnus.net/dox/modern-events-calendar/custom-fields/" target="_blank">' . __('How to set Custom Fields', 'mec') . ' </a>'
				.'</p></div>';
		} else {
