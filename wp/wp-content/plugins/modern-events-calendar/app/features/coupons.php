...
class MEC_feature_coupons extends MEC_base
{
        ...
        else
        {
            ...
            foreach($transaction['price_details']['details'] as $detail)
            {
                $price_details .= '<li class="mec-book-price-detail mec-book-price-detail-type'.$detail['type'].'">
                    ...
                    <span class="mec-book-price-detail-amount">'.$this->main->render_price($detail['amount'], $event_id).'</span>
                </li>';
            }
            
            $this->main->response(array(
                'success'=>1,
                'message'=>sprintf(__('Coupon valid and you get %s discount.', 'mec'), $this->main->render_price($discount, $event_id)),
                'data'=>array(
                    'discount'=>$discount,
                    'price_raw'=>$transaction['price'],
                    'price'=>$this->main->render_price($transaction['price'], $event_id),
    ...
    public function adjust_booking_fees()
    {
        ...
        $this->main->response(array('success'=>1, 'data'=>array('price_raw'=>$transaction['price'], 'price'=>$this->main->render_price($transaction['price'], $event_id), 'price_details'=>$price_details, 'transaction_id'=>$transaction_id)));
    }
}