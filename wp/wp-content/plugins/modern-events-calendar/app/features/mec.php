...
class MEC_feature_mec extends MEC_base
{
    ...
    public function menus()
    {
        ...
        add_submenu_page('mec-intro', __('Tags', 'mec'), __('Tags', 'mec'), 'edit_others_posts', 'edit-tags.php?taxonomy='.apply_filters('mec_taxonomy_tag', '').'&post_type='.$this->PT);
        add_submenu_page('mec-intro', esc_html($this->main->m('taxonomy_categories', __('Categories', 'mec'))), esc_html($this->main->m('taxonomy_categories', __('Categories', 'mec'))), 'edit_others_posts', 'edit-tags.php?taxonomy=mec_category&post_type='.$this->PT);
        add_submenu_page('mec-intro', esc_html($this->main->m('taxonomy_labels', __('Labels', 'mec'))), esc_html($this->main->m('taxonomy_labels', __('Labels', 'mec'))), 'edit_others_posts', 'edit-tags.php?taxonomy=mec_label&post_type='.$this->PT);
        add_submenu_page('mec-intro', esc_html($this->main->m('taxonomy_locations', __('Locations', 'mec'))), esc_html($this->main->m('taxonomy_locations', __('Locations', 'mec'))), 'edit_others_posts', 'edit-tags.php?taxonomy=mec_location&post_type='.$this->PT);
        add_submenu_page('mec-intro', esc_html($this->main->m('taxonomy_organizers', __('Organizers', 'mec'))), esc_html($this->main->m('taxonomy_organizers', __('Organizers', 'mec'))), 'edit_others_posts', 'edit-tags.php?taxonomy=mec_organizer&post_type='.$this->PT);

        // Speakers Menu
        if(isset($this->settings['speakers_status']) and $this->settings['speakers_status'])
        {
            add_submenu_page('mec-intro', esc_html($this->main->m('taxonomy_speakers', __('Speakers', 'mec'))), esc_html($this->main->m('taxonomy_speakers', __('Speakers', 'mec'))), 'edit_others_posts', 'edit-tags.php?taxonomy=mec_speaker&post_type='.$this->PT);
        }

        $capability = (current_user_can('administrator') ? 'manage_options' : 'mec_shortcodes');
        add_submenu_page('mec-intro', __('Shortcodes', 'mec'), __('Shortcodes', 'mec'), $capability, 'edit.php?post_type=mec_calendars');

        // Auto Email Menu
        if(isset($this->settings['auto_emails_module_status']) and $this->settings['auto_emails_module_status'])
        {
            $capability = 'manage_options';
            add_submenu_page('mec-intro', __('Emails', 'mec'), __('Emails', 'mec'), $capability, 'edit.php?post_type=mec-emails');
        }

        $capability = (current_user_can('administrator') ? 'manage_options' : 'mec_settings');
        add_submenu_page('mec-intro', __('MEC - Settings', 'mec'), __('Settings', 'mec'), $capability, 'MEC-settings', array($this, 'page'));

        add_submenu_page('mec-intro', __('MEC - Addons', 'mec'), __('Addons', 'mec'), 'manage_options', 'MEC-addons', array($this, 'addons'));
        add_submenu_page('mec-intro', __('MEC - Wizard', 'mec'), __('Wizard', 'mec'), 'manage_options', 'MEC-wizard', array($this, 'setup_wizard'));
    ...
    public function display_custom_data_field($skin, $value = 0)
    {
        return '<div class="mec-form-row mec-display-organizer-wrap mec-switcher">
            <div class="mec-col-4">
                <label for="mec_skin_'.$skin.'_custom_data">'.__('Display Custom Fields', 'mec').'</label>
            </div>
            <div class="mec-col-4">
    ...
    public function display_detailed_time_field($skin, $value = 0)
    {
        return '<div class="mec-form-row mec-switcher">
            <div class="mec-col-4">
                <label for="mec_skin_'.$skin.'_detailed_time">'.__('Detailed Time', 'mec').'</label>
                <span>'.__('For Multiple Day Events', 'mec').'</span>
            </div>
            <div class="mec-col-4">
            ...
            jQuery(document).ready(function()
            {
                ...
                var mecSellsChart = new Chart(ctx,
                {
                    ...
                    data:
                    {
                        ...
                        datasets: [
                        {
                            label: "'.esc_js(sprintf(__('Total Sales (%s)', 'mec'), $currency)).'",
                            data: ['.trim($stats, ', ').'],
    ...
    {
        add_meta_box('mec_metabox_app', __('Include MEC Assets', 'mec'), array($this, 'meta_box_assets_per_page'), 'page', 'side', 'low');
    }
    ...
    public function meta_box_assets_per_page($post)
    {
        ...
        <div class="mec-assets-per-page-metabox">
            <label for="mec_include_assets">
                ...
                <?php _e('Include Modern Events Calendar Assets (CSS, JavaScript, etc files.)', 'mec'); ?>
            </label>
        </div>
        <?php
    }
