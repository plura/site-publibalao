...
$export_attendees = (isset($ix['sync_g_export_attendees']) ? $ix['sync_g_export_attendees'] : 0);
...
foreach($mec_events as $mec_event)
{
    ...
    $gmt_offset = $main->get_gmt_offset($mec_event_id, $date['start']['timestamp']);
    ...
    {
        $attendees = array();
        foreach($main->get_event_attendees($data->ID) as $att)
        {
